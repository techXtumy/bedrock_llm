...
```bash
poetry install
poetry run python -m bedrock_be
```
...
```bash
BEDROCK_BE_RELOAD="True"
BEDROCK_BE_PORT="8000"
BEDROCK_BE_ENVIRONMENT="dev"
```
...
```
docker run -p "5432:5432" -e "POSTGRES_PASSWORD=bedrock_be" -e "POSTGRES_USER=bedrock_be" -e "POSTGRES_DB=bedrock_be" postgres:16.3-bullseye
```
