...
# Create a LLM client
client = LLMClient(region_name="us-east-1", model_name=ModelName.JAMBA_1_5_LARGE)

# Create a configuration for inference parameters
config = ModelConfig(temperature=0.2, top_p=0.9, max_tokens=1024)
...
async def knowledge_base_retrieve(query: str):
    ...
    )  # Make sure the return knowledge base is in string


async def main():
    cprint("Getting data from knowledge base ...", "cyan")
    # Query the user question into the knowledge base
    result = await knowledge_base_retrieve("What is eximbank?")
    ...
    prompt = MessageBlock(role="user", content="What is eximbank?" + "\n\n" + result)
