...
Contains message block types for different kinds of content:
- TextBlock: Basic text content
- ToolUseBlock: Tool invocation details
- ToolResultBlock: Tool execution results
- ImageBlock: Image data and metadata
"""
...
class SystemBlock(BaseModel):
    ...
    cache_control: Optional[CacheControl] = Field(
        description="Controls caching behavior for the system prompt"
    )
...
class TextBlock(BaseModel):
    ...
    cache_control: Optional[CacheControl] = Field(
        description="Controls caching behavior for the text block"
    )
    type: str = Field(description="Type of block, must be 'text'")
...
class ImageBlock(BaseModel):
    ...
    cache_control: Optional[CacheControl] = Field(
        description="Controls caching behavior for the image block"
    )
...
class ToolUseBlock(BaseModel):
    ...
    cache_control: Optional[CacheControl] = Field(
        description="Controls caching behavior for the tool use block"
    )
...
class ToolResultBlock(BaseModel):
    ...
    cache_control: Optional[CacheControl] = Field(
        description="Controls caching behavior for the tool result block"
    )
...
class ToolCallBlock(BaseModel):
    ...
    type: Optional[str] = Field(description="Type of the function")
...
class MessageBlock(BaseModel):
    ...
    name: Optional[str] = Field(
        description="Name of the function that you return when calling"
    )
    tool_calls: Optional[List[ToolCallBlock]] = Field(
        description="List of tool calls to make"
    )
    tool_call_id: Optional[str] = Field(
        description="ID of the tool call after running the tool"
    )
